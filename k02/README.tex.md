...
## ソースコードの説明
質問
  math.hの関数を導入しているのにM_PIが定義されてないと出力されます。#defineで無理やり定義しましたが、なぜですか。
ソースコードの説明
  A県とB県の標準偏差と平均を定義
  それぞれのXの値をだす
  それをp_stdnormにいれて値を出し
...
## 修正履歴
u1,u2,b1,b2が特に初期化されていないので計算がおかしくなっています。これら4つにはどのような値が入るのかを考えてみましょう。

[comment #20200804 sonoda]
- `M_PIが定義されていない`というワーニングは，math.hで円周率を定数として扱うのを推奨していないからです．厳密な計算でなければ気にしなくて結構です．
- OKです．課題2完了．